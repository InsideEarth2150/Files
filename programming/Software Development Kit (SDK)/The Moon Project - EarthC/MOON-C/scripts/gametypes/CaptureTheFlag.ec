        ...
        for(i=0;i<15;i=i+1)
        {
                        ...
                        if(rPlayer!=null) 
            {
                ...
                if (!rPlayer.GetNumberOfUnits() && !rPlayer.GetNumberOfBuildings())
                    rPlayer.CreateDefaultUnit(rPlayer.GetStartingPointX()+1,rPlayer.GetStartingPointY(),0);
                
                if(rPlayer.GetRace()==1)
                    CreateArtefact("NEABANNERUCS",rPlayer.GetStartingPointX(),rPlayer.GetStartingPointY(),0,i,artefactSpecialAIOther,i);
                if(rPlayer.GetRace()==2)
                    CreateArtefact("NEABANNERED",rPlayer.GetStartingPointX(),rPlayer.GetStartingPointY(),0,i,artefactSpecialAIOther,i);
                if(rPlayer.GetRace()==3)
                    CreateArtefact("NEABANNERLC",rPlayer.GetStartingPointX(),rPlayer.GetStartingPointY(),0,i,artefactSpecialAIOther,i);
